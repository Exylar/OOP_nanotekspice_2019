...
Test(inputs_Nand_gates, allUndifined)
{
    OSRedirector oss(std::cout);
    Circuit circuit;
    Parser parser(&circuit);

    parser.checkFile("./tests/nts_files/combinatory/5_input_nand_gate.nts");
    parser.parseNtsFile("./tests/nts_files/combinatory/5_input_nand_gate.nts");
    circuit.orderOutput();
    circuit.simulate();
...
Test(inputs_Nand_gates, a0b0c0d0e0)
{
    OSRedirector oss(std::cout);
    Circuit circuit;
    Parser parser(&circuit);
...
Test(inputs_Nand_gates, a1b1c1d1e1)
{
    OSRedirector oss(std::cout);
    Circuit circuit;
    Parser parser(&circuit);
